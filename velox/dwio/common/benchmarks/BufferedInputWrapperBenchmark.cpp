...
using dwio::common::bench::clearBenchmarkCacheRoot;
...
using dwio::common::bench::Workload;
...
PassResult runWrapper(Harness& h, const CellSpec& spec, Phase phase) {
  ...
  if (phase != Phase::kHot) {
    ...
    while (warmed < targetKeys) {
      ...
      WorkloadDriver warm{
          Workload::Sequential, n, spec.readSize,
  ...
  for (int p = 0; p < FLAGS_measure_passes; ++p) {
    ...
    if (scrubKeys > 0) {
      WorkloadDriver scrub{
          Workload::Sequential, scrubKeys, spec.readSize,
...
void cleanup() {
  ...
  // cleanup() runs from normal shutdown and from the SIGINT handler; the
  // sentinel-aware clear can throw, so log-and-continue instead of letting an
  // exception escape a signal-driven teardown.
  try {
    clearBenchmarkCacheRoot(FLAGS_ssd_path);
  } catch (const std::exception& e) {
    LOG(ERROR) << "[cleanup] ssd cache-root clear failed: " << e.what();
  }
  try {
    clearBenchmarkCacheRoot(FLAGS_filecache_root);
  } catch (const std::exception& e) {
    LOG(ERROR) << "[cleanup] filecache cache-root clear failed: " << e.what();
  }
}
...
int main(int argc, char** argv) {
  ...
  // with '/'); the --ssd_path default is CWD-relative (never /tmp), so resolve
  // it before the cbi harness hands it to SsdCache.
  harnessConfig.ssdPath = std::filesystem::absolute(FLAGS_ssd_path).string();
  ...
  const auto workloads = parseCsv<Workload>(
