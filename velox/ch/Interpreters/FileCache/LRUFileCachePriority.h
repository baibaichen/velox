...
/// ClickHouse Clang thread-safety annotations (`base/base/defines.h`). They are
/// compile-time-only hints with no runtime effect; the Velox port has no shared
/// `defines.h`, so define them as no-ops here. `#ifndef`-guarded so a future
/// shared shim can override them without a redefinition conflict.
...
#define TSA_REQUIRES(...)
#endif
#ifndef TSA_SUPPRESS_WARNING_FOR_READ
