...
{
    /// ClickHouse uses `magic_enum::enum_name`; the reviewed switch helper keeps
    /// the same names without depending on magic_enum.
    switch (kind)
    {
...
void FileSegment::write(char * from, size_t size, size_t offset_in_file)
{
    ...
    DownloadState * download = nullptr;
    {
        ...
        if (download_state != State::DOWNLOADING)
            VELOX_FAIL("Expected DOWNLOADING state, got {}", stateToString(download_state));
    ...
    try
    {
        ...
        /// which is only executed in debug/sanitizer builds.
        std::lock_guard debug_write_lock(download->write_mutex);
#endif

        if (!download->cache_writer)
        {
            /// The manager-injected write-file factory decides append-vs-create from the
            /// existing downloaded size: downloaded_size == 0 creates a new file (an existing
            /// path is an error), downloaded_size > 0 opens the partial file positioned at its
            /// end. This replaces CH's flag computation (`O_WRONLY | O_APPEND | O_CLOEXEC`).
            download->cache_writer = cache->createCacheWriteBuffer(getPath(), downloaded_size);
        }

        FAIL_POINT_TRIGGER(cache_filesystem_failure);
        ...
        chassert(fs::file_size(file_segment_path) == downloaded_size);
    }
    catch (FileCacheErrnoException & e)
    {
        const int code = e.getErrno();
        const bool is_no_space_left_error = code == ENOSPC || code == EDQUOT;
        ...
        if (fs::exists(file_segment_path))
        {
            ...
            {
                const size_t file_size = fs::file_size(file_segment_path);
    ...
    {
        /// Any non-space or non-errno failure: mark the download failed and rethrow the
        /// original exception without reconciling downloaded_size against the on-disk size.
        /// A catch-all reconciliation would be a forbidden fallback that masks corruption,
        /// permission, or hardware errors behind a size fix-up that looks like a partial write.
        auto lk = lock();
        setDownloadFailedUnlocked(lk);
        throw;
    }
...
FileSegment::State FileSegment::wait(size_t offset, const folly::CancellationToken & cancellation_token)
{
    ...
    if (download_state == State::DOWNLOADING)
    {
        ...
        /// stalled or dead downloader would otherwise pin this thread until the full timeout.
        /// CH reads the QueryStatus from CurrentThread; Velox injects an explicit cancellation token
        /// (see design 09), and a cancellation raises a Velox exception rather than continuing.
        ...
        while (true)
        {
            if (cancellation_token.isCancellationRequested())
                VELOX_FAIL("Query was cancelled while waiting for a file segment download");
...
KeyMetadataPtr FileSegment::getKeyMetadata() const
{
    ...
    if (metadata)
        return metadata;
    VELOX_FAIL("Cannot lock key, key metadata is not set ({})", stateToString(download_state));
}
...
void FileSegment::setDownloadedUnlocked(const FileSegmentGuard::Lock & lock)
{
    ...
    download_state = State::DOWNLOADED;
    download_finished_time = std::chrono::duration_cast<std::chrono::seconds>(
...
void FileSegment::renameToIncludeSizeInNameUnlocked(const FileSegmentGuard::Lock &)
{
    ...
    try
    {
        ...
        /// the opened-file cache keyed by `old_path`. When this segment is later removed,
        ...
        /// only prevents future opens from reusing the pre-rename descriptor. The manager-owned
        /// opened-file cache keys by path (all handle/flag variants), replacing CH's per-flag removals.
        if (cache)
            cache->invalidateOpenedFile(old_path);
    }
    catch (...)
    {
...
bool FileSegment::assertCorrectnessUnlocked(const FileSegmentGuard::Lock & lock) const
{
    ...
    switch (download_state.load())
    {
        ...
        case State::DOWNLOADED:
        {
            ...
            if (!size_in_filename)
            {
                [[maybe_unused]] auto file_size = fs::file_size(getPath());
...
{
    return facebook::velox::ch::toString(file_segments, with_state);
}
