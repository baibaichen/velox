...
{
    /// D-011-3: CH guards this bookkeeping allocation with
    /// `LockMemoryExceptionInThread(VariableContext::Global)` to suppress
    /// memory-tracker exceptions. Velox has no CH memory tracker, so there is
    /// nothing to block here; the guard is omitted per the approved mapping.
    size_t prev = 0;
    {
...
size_t LRUFileCachePriority::removeInvalidatedEntries(size_t max_batch, CachePriorityGuard & cache_guard)
{
    ...
    while (removed < max_batch)
    {
        ...
        catch (...)
        {
            ...
            /// D-011-3: CH additionally guards the `push_front` allocation with
            /// `LockMemoryExceptionInThread(VariableContext::Global)` to suppress
            /// `MEMORY_LIMIT_EXCEEDED`. Velox has no CH memory tracker, so the guard
            /// is omitted per the approved mapping; the surrounding logic is unchanged.
            std::lock_guard cleanup_lock(invalidated_mutex);
...
void LRUFileCachePriority::shuffle(const CachePriorityGuard::WriteLock &)
{
    ...
    for (auto it = queue.begin(); it != queue.end(); ++it)
        its.push_back(it);
    /// D-011-2: CH seeds `pcg64` from `randomSeed()`; Velox uses `folly::Random`.
    /// `folly::ThreadLocalPRNG` is a UniformRandomBitGenerator suitable for std::shuffle.
    folly::ThreadLocalPRNG generator;
...
LRUFileCachePriority::LRUQueue::iterator & LRUFileCachePriority::evictionPos(EvictionCursor cursor)
{
    switch (cursor)
    {
        ...
        case EvictionCursor::FromHead:
            throwFileCacheException("EvictionCursor::FromHead has no persistent cursor");
    }
    throwFileCacheException("Unknown EvictionCursor");
}
