...
LRUFileCachePriority::~LRUFileCachePriority()
{
    ...
    /// priority is pruned by the per-user overcommit snapshot, or when a whole cache is
...
{
    /// CH suppresses the per-thread MemoryTracker exception here so this noexcept
    /// path cannot throw during the `push_back` allocation. Velox has no per-thread
    /// MemoryTracker, so the guard is a no-op and is omitted.
...
size_t LRUFileCachePriority::removeInvalidatedEntries(size_t max_batch, CachePriorityGuard & cache_guard)
{
    ...
    while (removed < max_batch)
    {
        ...
        catch (...)
        {
            ...
            /// CH suppresses `MEMORY_LIMIT_EXCEEDED` from the `push_front` allocation via
            /// the per-thread MemoryTracker guard; Velox has no such tracker, so it is omitted.
            std::lock_guard cleanup_lock(invalidated_mutex);
...
{
    /// Once the atomic decrements below reach 0, the per-user overcommit snapshot
...
LRUFileCachePriority::LRUQueue::iterator & LRUFileCachePriority::evictionPos(EvictionCursor cursor)
{
    switch (cursor)
    {
        ...
        case EvictionCursor::FromHead:
            VELOX_FAIL("EvictionCursor::FromHead has no persistent cursor");
    }
    VELOX_FAIL("EvictionCursor::FromHead has no persistent cursor");
}
