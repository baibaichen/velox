...
#include "velox/ch/Interpreters/FileCache/CacheUsage.h"
...
/// ClickHouse's `SCOPE_EXIT` maps to folly's scope guard (already provided by
/// `folly/ScopeGuard.h`), which runs on both normal and exceptional exit
/// (approved cross-profile mapping). folly uses the `SCOPE_EXIT { ... };`
/// spelling, so the call sites below are written in that form.
...
/// The two wall-clock measurement sites use the approved
/// `DeltaCpuWallTimeStopWatch` mapping; only one wall snapshot is read
/// (`elapsed().wallNanos / 1'000'000`), no stop/reset/restart contract.
using Stopwatch = facebook::velox::DeltaCpuWallTimeStopWatch;

/// ClickHouse `tryParseUInt64(out, str)` replacement (returns false on any
/// non-numeric or partial parse; no exception).
bool tryParseUInt64(UInt64 & out, const std::string & s)
{
    if (s.empty())
        return false;
    errno = 0;
...
std::string FileCacheReserveStat::Stat::toString() const
{
    return fmt::format(
        ...
        releasable_size, releasable_count, non_releasable_size, non_releasable_count,
        evicting_count, moving_count, invalidated_count, candidates_iteration_steps, clients_iterated);
}
...
FileCache::CheckCacheProbability::CheckCacheProbability(double probability, UInt64 seed)
    : rndgen(seed == 0 ? std::random_device{}() : seed)
...
FileCache::FileCache(
    ...
    , common_origin(common_origin_)
    , create_write_file(create_write_file_)
    , open_read_file(open_read_file_)
    , invalidate_opened_file_callback(invalidate_opened_file_)
    ...
    , metadata(settings.path,
               ...
               worker_pool_,
               memory_pool_,
               /* reserve_space_wait_lock_timeout_milliseconds */ 1000,
               invalidate_opened_file_ ? invalidate_opened_file_ : OpenedFileInvalidator([](const std::string &) {}),
               common_origin_.user_id)
    , check_cache_probability(settings.checkCacheProbability)
{
    ...
    switch (settings.cachePolicy)
    {
        ...
        case FileCachePolicy::LRU_OVERCOMMIT:
        case FileCachePolicy::SLRU_OVERCOMMIT:
            VELOX_FAIL("Overcommit cache policies are not supported without distributed cache");
#endif
    }
    if (use_split_cache)
    {
        ...
        const auto policy = settings.cachePolicy;
        if (policy == FileCachePolicy::LRU_OVERCOMMIT || policy == FileCachePolicy::SLRU_OVERCOMMIT)
            VELOX_FAIL("`use_split_cache` is not supported with overcommit cache policies");
    ...
    LOG_DEBUG(log, "Using {} cache policy", settings.cachePolicy);
    ...
    const auto cache_policy = settings.cachePolicy;
    ...
    CurrentMetrics::add(CurrentMetrics::FilesystemCacheSizeLimit, settings.maxSize);
}
...
FileSegment::LocalCacheWriterPtr FileCache::createCacheWriteBuffer(const std::string & path, size_t existing_size) const
{
    /// Manager-injected write-file factory replaces CH's global FileSystem.
    /// `existing_size > 0` opens the partial file positioned at its end (append);
    /// `existing_size == 0` creates a new file (an existing path is an error).
    /// The writer is external-only (bufferSize 0): FileSegment::write feeds the
    /// data pointer through `set(from, size, size)` and flushes with `next()`.
    auto write_file = create_write_file(path, /* append */ existing_size > 0);
    return std::make_shared<WriteBufferFromVeloxWriteFile>(std::move(write_file), memory_pool, /* bufferSize */ 0);
}

FileSegment::RemoteFileReaderPtr FileCache::createCacheReadBuffer(const std::string & path) const
{
    /// Manager-injected read-file factory replaces CH's global FileSystem.
    auto read_file = open_read_file(path);
    return std::make_shared<ReadBufferFromVeloxReadFile>(std::move(read_file), memory_pool);
}

void FileCache::invalidateOpenedFile(const std::string & path) const
{
    /// Manager-owned opened-file cache invalidation (replaces CH's
    /// OpenedFileCache::instance().remove()); no-op if no callback was injected.
    if (invalidate_opened_file_callback)
        invalidate_opened_file_callback(path);
}
...
void FileCache::initializeImpl(bool load_metadata)
{
    ...
    {
        eviction_pool = std::make_unique<FileCacheThreadPool>(
            worker_pool,
            /* max_threads */ keep_up_free_space_eviction_threads,
            /* queue_size */ keep_up_free_space_eviction_threads);

        keep_up_free_space_ratio_task = scheduler.createTask("FileCache:" + name + ":free-space", [this] { freeSpaceRatioKeepingThreadFunc(); });
...
void FileCache::freeSpaceRatioKeepingThreadFunc()
{
    ...
    ProfileEvents::increment(ProfileEvents::FilesystemCacheFreeSpaceKeepingThreadWorkMilliseconds, watch.elapsed().wallNanos / 1'000'000);
...
void FileCache::freeSpaceRatioImpl(size_t & reschedule_ms)
{
    ...
    try
    {
        ...
        {
            /// Increment before the remover task becomes visible to a worker: otherwise a
            /// remover could observe a transient running_removers==... and wrongly conclude
            /// it is the last one. Roll back on submission failure.
            ++running_removers;
            try
            {
            ...
            /// Only mark that a remover was actually scheduled after the submit
            /// succeeded, so a failed first submit does not leave the collector
            /// waiting on a finalization queue that no remover will ever finish().
...
void FileCache::backgroundCleanupTaskFunc()
{
    size_t removed = 0;
    {
        ...
        ProfileEvents::increment(
            ProfileEvents::FilesystemCacheInvalidatedEntriesCleanupThreadWorkMilliseconds, watch.elapsed().wallNanos / 1'000'000);
    }
...
void FileCache::loadMetadataImpl()
{
    auto parse_user = [&](const fs::path & path) -> std::optional<OriginInfo>
    {
        ...
        return OriginInfo(filename.substr(0, pos), std::stoull(filename.substr(pos + 1)));
    };
    ...
    /// The listing/loading workers run on the manager-injected shared worker pool
    /// (CH used unbounded ThreadFromGlobalPool). They block on `key_dirs_queue` and
    /// coordinate termination via finish(): every one of them must be able to run
    /// concurrently, otherwise a pool smaller than the total worker count starves
    /// (loaders parked in pop() occupy every slot, so no lister ever calls finish()).
    /// In asynchronous mode the `load_metadata_main_thread` itself occupies one pool
    /// slot while it blocks in the joins below, so it needs one extra thread.
    ///
    /// The pool is SHARED across every cache the manager owns. FileCache must not
    /// resize it here: a temporary grow/restore read-modify-write races other caches
    /// loading concurrently (two caches would each read the same "previous" size and
    /// the second SCOPE_EXIT would shrink the pool below what a still-running cache
    /// needs). The Task-013 manager already budgets the pool for the sum of every
    /// unique cache's loadMetadataThreads plus its async slot (see
    /// port/task/013-filecache-factory-manager.md "compute workerPoolMax"), so the
    /// capacity is guaranteed to be present. Verify it fail-closed instead of
    /// mutating the shared pool: a pool too small to run all workers concurrently
    /// would deadlock, so refuse with an actionable required-vs-available error.
    const size_t total_load_threads = static_cast<size_t>(num_listing_threads + num_loading_threads)
        + (load_metadata_asynchronously ? 1 : 0);
    const size_t available_pool_threads = worker_pool.numThreads();
    if (available_pool_threads < total_load_threads)
        VELOX_FAIL(
            "FileCache '{}' metadata load needs {} shared worker-pool thread(s) "
            "({} listing + {} loading{}) but the pool provides only {}. The pool is "
            "sized by the manager to the aggregate of every cache's load-metadata "
            "budget; a smaller pool would deadlock the listing/loading coordination.",
            name,
            total_load_threads,
            num_listing_threads,
            num_loading_threads,
            load_metadata_asynchronously ? " + 1 async load thread" : "",
            available_pool_threads);
    ...
    FileCacheBoundedQueue<std::pair<fs::path, OriginInfo>> key_dirs_queue(num_loading_threads == 0 ? 0 : 1000);
...
void FileCache::onSegmentEvicted(const FileSegment & segment, const String & user_id) const
{
    ...
    /// Per-cache and per-user Prometheus eviction metrics are deferred to Task 017
    /// (kept as no-op name-surface shims here). The ProfileEvents increments above
    /// preserve the counting control flow.
    (void)user_id;
}
...
void FileCache::applySettingsIfPossible(const FileCacheSettings & new_settings, FileCacheSettings & actual_settings)
{
        ...
        && metadata.setBackgroundDownloadQueueSizeLimit(new_settings.backgroundDownloadQueueSizeLimit))
    {
        LOG_INFO(log, "Changed background_download_queue_size from {} to {}",
                 actual_settings.backgroundDownloadQueueSizeLimit,
                 new_settings.backgroundDownloadQueueSizeLimit);
    ...
    if (new_settings.backgroundDownloadThreads != actual_settings.backgroundDownloadThreads)
    {
        ...
        if (updated)
        {
            LOG_INFO(log, "Changed background_download_threads from {} to {}",
                    actual_settings.backgroundDownloadThreads,
                    new_settings.backgroundDownloadThreads);
    ...
    if (new_settings.backgroundDownloadMaxFileSegmentSize != actual_settings.backgroundDownloadMaxFileSegmentSize)
    {
        ...
        LOG_INFO(log, "Changed background_download_max_file_segment_size from {} to {}",
                actual_settings.backgroundDownloadMaxFileSegmentSize,
                new_settings.backgroundDownloadMaxFileSegmentSize);
    ...
    if (new_settings.reserveGranularity != actual_settings.reserveGranularity)
    {
        ...
        LOG_INFO(log, "Changed reserve_granularity from {} to {}",
                actual_settings.reserveGranularity,
                new_settings.reserveGranularity);
    ...
    if (new_settings.idleClientTtlSec != actual_settings.idleClientTtlSec)
    {
        ...
        LOG_INFO(log, "Changed idle_client_ttl_sec from {} to {}",
                 actual_settings.idleClientTtlSec, new_ttl);
    ...
    if (new_settings.idleClientCheckIntervalSec != actual_settings.idleClientCheckIntervalSec)
    {
        ...
        LOG_INFO(log, "Changed idle_client_check_interval_sec from {} to {}",
                 actual_settings.idleClientCheckIntervalSec,
                 new_settings.idleClientCheckIntervalSec);
...
FileCache::SizeLimits FileCache::doDynamicResize(const SizeLimits & prev_limits, const SizeLimits & desired_limits)
{
    if (prev_limits.slru_size_ratio != desired_limits.slru_size_ratio)
        VELOX_FAIL("Dynamic resize of size ratio is not allowed");
    ...
    bool modified_size_limit = false;
    {
        ...
        if (prev_limits.max_size != main_priority->getSizeLimit(cache_lock))
            VELOX_FAIL("Current limits inconsistency in size");

        if (prev_limits.max_elements != main_priority->getElementsLimit(cache_lock))
            VELOX_FAIL("Current limits inconsistency in elements number");
...
FileCache::QueryContextHolderPtr FileCache::getQueryContextHolder(
    const String & query_id, const FileCacheReadOptions & options)
{
    if (!query_limit || options.maxDownloadSizePerQuery == 0)
        return {};

    auto lock = cache_guard.writeLock();
    auto context = query_limit->getOrSetQueryContext(query_id, options, lock);
