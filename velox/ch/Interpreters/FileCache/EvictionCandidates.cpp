...
std::string QueueEvictionInfo::toString() const
{
    std::string result = fmt::format(
        "description: {}, user: {}, size to evict: {}, elements to evict: {}",
        description, user_id, size_to_evict, elements_to_evict);
    if (hold_space)
    {
        result += fmt::format(
            ", hold space size: {}, hold space elements: {}",
            hold_space->getSize(), hold_space->getElements());
    }
    return result;
}
...
{
    std::string result = fmt::format(
        "total size to evict: {}, total elements to evict: {}, total hold size: {}, total hold elements: {}",
        size_to_evict, elements_to_evict, hold_size, hold_elements);
    for (const auto & [queue_id, info] : *this)
        result += fmt::format(", [queue id {}, {}]", queue_id, info->toString());
    return result;
}
