...
{
    /// D-011-4: CH builds this diagnostic string with `WriteBufferFromOwnString`;
    /// Velox uses `std::ostringstream` (a debug/diagnostic string builder only).
...
{
    /// D-011-4: see QueueEvictionInfo::toString.
    std::ostringstream wb;
    ...
    bool replace_if_exists)
{
    ...
    if (!replace_if_exists)
        throwFileCacheException("Queue with id {} already exists", queue_id);
...
void EvictionCandidates::evict()
{
    ...
    /// D-011-1: CH scopes a per-thread ProfileEvents timer here via
    /// `CurrentThread::getProfileEvents().timer(...)`; Velox uses the Task-003
    /// `ProfileEventTimeIncrement<Microseconds>` no-op RAII shim. Real timing is Task 017.
    ProfileEventTimeIncrement<Microseconds> timer(ProfileEvents::FilesystemCacheEvictMicroseconds);
