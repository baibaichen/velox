    ...
    bool is_initial_load) const
{
    ...
    if (reservee)
    {
        const auto * slru_iterator = static_cast<SLRUIterator *>(reservee->getNestedOrThis());
    ...
    const CacheStateGuard::Lock & lock)
{
    ...
    if (reservee)
    {
        slru_iterator = static_cast<SLRUIterator *>(reservee->getNestedOrThis());
    ...
    CacheStateGuard & state_guard)
{
    ...
    auto * slru_iterator = static_cast<SLRUIterator *>(reservee->getNestedOrThis());
    bool success = false;
    ...
    CacheStateGuard & state_guard)
{
    ...
    res.addAfterEvictWriteCallback([=, this](const CachePriorityGuard::WriteLock & lk) mutable
    {
        for (auto & [key, key_candidates] : *downgrade_candidates)
        {
            while (!key_candidates.candidates.empty())
            {
                ...
                auto * slru_iterator = static_cast<SLRUIterator *>(iterator->getNestedOrThis());
    ...
    res.addAfterEvictStateCallback([=, this](const CacheStateGuard::Lock & lk)
    {
        ...
        while (true)
        {
            ...
            try
            {
                iterator = static_cast<SLRUIterator *>(info->slru_iterator->getNestedOrThis());
                chassert(iterator);
