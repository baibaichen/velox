...
namespace
{
    /// D-011-6: debug-checked downcast matching CH `assert_cast` semantics —
    /// exact-type check via `dynamic_cast` in debug/sanitizer builds, plain
    ...
    To assert_cast(From * from)
    {
        ...
        VELOX_CHECK_NOT_NULL(result, "assert_cast: bad downcast");
        return result;
#else
    ...
    bool is_initial_load)
{
    ...
    if (is_initial_load)
    {
        chassert(size);
        if (!state_lock)
            throwFileCacheException("Startup initialization requires state lock");
    ...
    CacheStateGuard & state_guard)
{
    ...
    {
        /// D-011-5: CH injects a `std::bernoulli_distribution`/`thread_local_rng`
        /// fault here inside `fiu_do_on(file_cache_slru_downgrade_fail_before_finalize)`.
        /// The failpoint is a no-op in this phase, so the whole fault-injection branch
        /// (including the live RNG) is dead. No RNG is emitted per the approved mapping.
...
SLRUFileCachePriority::EntryPtr SLRUFileCachePriority::SLRUIterator::getEntry() const
{
    ...
    if (!entry_ptr)
        throwFileCacheException("Entry pointer expired");
    return entry_ptr;
}
