...
/// Defined in `EvictionCandidates.h`; only referenced here by pointer/reference in private
/// method signatures, so a forward declaration keeps that (Task-011) header out of this apex.
class EvictionInfo;
...
struct FileCacheReserveStat
{
    ...
    Stat total_stat;
    /// CH `magic_enum::enum_count<FileSegmentKind>()`; explicit literal 2 (Regular, Ephemeral).
...
public:
    ...
    /// Common origin for this cache: instance method (host-injected stable commonUserId
    /// replaces CH's static ServerUUID-derived origin).
    const FileCacheOriginInfo & getCommonOrigin() const;
    ...
    QueryContextHolderPtr getQueryContextHolder(const String & query_id, const FileCacheReadOptions & settings);
...
private:
    ...
    /// B5: stable, host-injected common cache user id (replaces CH's ServerUUID-derived id).
    /// Non-empty and != "internal". Declared before `common_origin`, which caches it.
    const std::string common_user_id;
    /// Cached common origin built from `common_user_id` in the constructor; returned by
    ...
    /// D8: CH `OnceFlag` (`callOnce`) maps to `std::once_flag`/`std::call_once`
    /// (retry-on-exception semantics preserved).
    std::once_flag initialize_called;
    ...
    private:
        /// D-011-2 RNG mapping: CH `pcg64_fast`/`randomSeed()` maps to `folly::Random`
        /// (drawn at the call site in `FileCache.cpp`); only the distribution/seed are stored.
