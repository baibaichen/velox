...
namespace
{
/// CH used `CurrentThread::isInitialized() && tryGetQueryContext() && !getQueryId().empty()`.
/// In the Velox port the per-operation query identity comes from `FileCacheQueryIdScope`; a
/// query is "initialized" for the purposes of the per-query cache limit exactly when a scope
/// with a non-empty id is active on the current thread.
    ...
    const CachePriorityGuard::WriteLock & lock)
{
    ...
    if (!inserted)
    {
        ...
        throwFileCacheException(
            ...
            offset, key_metadata->key.toString());
    }
}
    ...
    const CachePriorityGuard::WriteLock & lock)
{
    ...
    if (record == records.end())
        throwFileCacheException("There is no {}:{} in query context", key.toString(), offset);
