...
/// ClickHouse formats key state with magic_enum::enum_name; the reviewed helper
/// keeps the same names without depending on magic_enum.
...
/// Mirrors ClickHouse's DBMS_DEFAULT_BUFFER_SIZE (1 MiB), the initial reusable
/// background-download buffer size (grown only lazily, once per worker).
constexpr size_t DBMS_DEFAULT_BUFFER_SIZE = 1048576;
...
CacheMetadata::OriginInfoPtr CacheMetadata::getOrCreateSharedOrigin(const OriginInfo & origin)
{
    ...
    return origins.withShard(pool_key, [&](auto & map) -> OriginInfoPtr
    {
        ...
        /// Returns a copied shared_ptr, never a reference/iterator into the shard
        /// map (SD1 no-reference-escape).
        return it->second;
    });
}
...
LockedKeyPtr KeyMetadata::lock()
{
    ...
    VELOX_FAIL("Cannot lock key {} (state: {})", key.toString(), keyStateName(key_state));
}
...
CacheMetadata::CacheMetadata(
    ...
    , invalidate_opened_file(std::move(invalidate_opened_file_))
    ...
    bool is_initial_load)
{
...

    {
        ...
        if (key_not_found_policy == KeyNotFoundPolicy::THROW)
            VELOX_FAIL("No such key `{}` in cache", key.toString());
    ...
    bool is_initial_load)
{
    KeyMetadataPtr result;
    {
        ...
        /// Copy the KeyMetadataPtr (a shared_ptr, stable across rehash) out of the
        /// bucket before releasing the per-bucket guard: no reference/iterator into
        /// the F14 bucket crosses the lock release (SD4).
        result = it->second;
    }
...
void CacheMetadata::iterate(IterateFunc && func, const KeyMetadata::UserID & user_id)
{
    for (auto & bucket : metadata_buckets)
    {
        ...
        for (auto & [key, key_metadata] : bucket)
        {
            ...
            VELOX_FAIL("Cannot lock key {}: key does not exist", key_metadata->key.toString());
        }
    }
}
...
public:
    ...
    bool next(Iterator::OnFileSegmentFunc func)
    {
        ...
        while (bucket_it != metadata_buckets.end())
        {
            ...
            for (const auto & [key, key_metadata] : *bucket_it)
            {
                if (!key_metadata->checkAccess(user_id))
                    continue;
                ...
                for (const auto & [offset, file_segment_metadata] : *key_metadata)
...
void CacheMetadata::removeKey(const Key & key, bool if_exists, const UserID & user_id)
{
    ...
    if (it == bucket.end())
    {
        if (if_exists)
            return;
        VELOX_FAIL("No such key: {}", key.toString());
    }
    ...
    if (state != KeyMetadata::KeyState::ACTIVE)
    {
        if (if_exists)
            return;
        VELOX_FAIL("No such key: {} (state: {})", key.toString(), keyStateName(state));
    }
    ...
    const CacheMetadataGuard::Lock &)
{
    ...
    if (!locked_key.empty())
        VELOX_FAIL("Cannot remove non-empty key: {}", key.toString());
...
/// Handwritten internal cleanup queue (NOT FileCacheBoundedQueue): deduplicates
/// keys with an F14FastSet, blocks the cleanup worker on a cv, and unblocks it on
/// cancel with notify_all so the join in shutdown() cannot hang.
class CleanupQueue
{
...
private:
    folly::F14FastSet<FileCacheKey, FileCacheKeyHash> keys;
...
/// Handwritten internal download queue (NOT FileCacheBoundedQueue): bounded
/// std::queue of DownloadInfo (which carries a weak_ptr<FileSegment> for
/// key/offset-reuse identity), mutex+cv blocking pop, cancel flag with
/// notify_all, and a live-updatable size limit.
class DownloadQueue
{
    friend class CacheMetadata;
...
{
    /// CH uses std::optional<Memory<>>; mapped to a reusable pool-charged BufferPtr
    /// (empty until first allocation, then grown once per worker).
    BufferPtr memory;
    while (true)
    {
        Key key;
...

        {
            ...
            key = entry.key;
            offset = entry.offset;
            file_segment_weak = entry.segment;
            ...
            {
                {
                    ...
                    /// The weak pointer guards against key/offset reuse: a new segment created at
                    /// the same offset after the original was deleted must not be mistaken for the
...
void CacheMetadata::downloadImpl(FileSegment & file_segment, BufferPtr & memory) const
{
    LOG_TEST(
    ...
    const auto reserve_space_lock_wait_timeout_milliseconds = reserve_space_wait_lock_timeout_milliseconds;
    ...
    while (size_to_download && !buf->eof())
    {
        ...
        catch (FileCacheErrnoException & e)
        {
            ...
            {
                LOG_INFO(log, "Insert into cache is skipped due to insufficient disk space.");
                break;
            }
            throw;
        }
    ...
    /// Reset buffer to avoid a stale offset mismatch between the remote reader and
    /// the file segment's current write offset.
...
void CacheMetadata::startup()
{
    ...
    for (size_t i = 0; i < download_threads_num; ++i)
    {
        ...
        download_threads.back()->thread = std::make_unique<FileCacheWorker>(
            worker_pool, [this, thread = download_threads.back()] { downloadThreadFunc(thread->stop_flag); });
    }
    cleanup_thread = std::make_unique<FileCacheWorker>(worker_pool, [this]{ cleanupThreadFunc(); });
}

void CacheMetadata::shutdown()
{
    /// Cancel both queues BEFORE joining the workers that pop from them: setting the
    /// cancel flag and calling notify_all first unblocks any worker parked in a
    /// blocking pop, so the subsequent join cannot hang.
    download_queue->cancel();
    cleanup_queue->cancel();

    /// Observable cancel-before-join checkpoint (TestValue seam; inert unless a
    /// test arms it; release-elided). Reached only after both queues are cancelled
    /// and before any worker is joined. A join-before-cancel regression blocks at
    /// the join below on an un-woken worker and never reaches this point, so a test
    /// that records the afterCancel/afterJoin order observes the exact ordering.
    facebook::velox::common::testutil::TestValue::adjust(
        "facebook::velox::ch::CacheMetadata::shutdown::afterCancelBeforeJoin", this);
    ...
    facebook::velox::common::testutil::TestValue::adjust(
        "facebook::velox::ch::CacheMetadata::shutdown::afterJoin", this);
}
...
void KeyMetadata::invalidateOpenedFile(const std::string & path) const
{
    /// Replaces CH's OpenedFileCache::instance().remove(...): the manager-owned
    /// opened-file cache invalidation callback is injected into CacheMetadata.
    if (cache_metadata->invalidate_opened_file)
        cache_metadata->invalidate_opened_file(path);
}
...
LockedKey::~LockedKey()
{
    ...
    /// If state is ACTIVE and the key turns out empty, we submit it for delayed removal,
    /// because we do not want to always lock all cache metadata when we remove file
    /// segments. Sometimes we do - we remove the empty key without delay - then the key
    /// state will be REMOVED here and we will return in the check above.
...
bool LockedKey::removeAllFileSegments()
{
    ...
    for (auto it = key_metadata->begin(); it != key_metadata->end();)
    {
        ...
        {
            /// File segment is currently a removal candidate, we do not know if it will be
            /// removed or not yet, but its size is currently accounted as potentially removed,
            /// so if we remove file segment now, we break the freeable_count calculation in
            /// tryReserve.
            ++it;
            removed_all = false;
            continue;
    ...
    bool invalidate_queue_entry)
{
    auto it = key_metadata->find(offset);
    if (it == key_metadata->end())
        VELOX_FAIL("There is no offset {} in key {}", offset, getKey().toString());
    ...
    bool invalidate_queue_entry)
{
    ...
    try
    {
        ...
        else if (fs::exists(path))
        {
            ...
            /// Clear the manager-owned opened-file cache to avoid reading from an
            /// incorrect file descriptor after the underlying file is removed.
            key_metadata->invalidateOpenedFile(path);
...
std::vector<FileSegment::Info> LockedKey::sync()
{
    ...
    for (auto it = key_metadata->begin(); it != key_metadata->end();)
    {
        ...
        {
            VELOX_FAIL("File segment has unexpected state: DETACHED ({})", file_segment->getInfoForLog());
        }
