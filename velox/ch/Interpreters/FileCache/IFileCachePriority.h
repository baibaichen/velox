...
public:
    IFileCachePriority(const IFileCachePriority &) = delete;
    IFileCachePriority & operator=(const IFileCachePriority &) = delete;
    ...
    struct Entry
    {
        ...
        /// Explicit exhaustive `State` name mapping (CH uses `magic_enum::enum_name`).
        static std::string_view toStringView(State state)
        {
            switch (state)
            {
                case State::Active: return "Active";
                case State::PreActive: return "PreActive";
        ...
        void resetFlag(State from_state, State to_state = State::Active) noexcept
        {
            ...
            chassert(
                prev == from_state,
                printUnexpectedState(prev, toStringView(from_state), fmt::format("{}", toStringView(to_state))));
        }
    ...
    private:
            ...
            State prev_state, std::string_view expected_state, std::string type) const
        {
            return fmt::format(
                ...
                toStringView(prev_state), expected_state, type, toString());
        }
    ...
    {
    public:
        HoldSpace(const HoldSpace &) = delete;
        HoldSpace & operator=(const HoldSpace &) = delete;
