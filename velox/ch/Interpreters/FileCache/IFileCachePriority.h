...
public:
    ...
    struct Entry
    {
        ...
        /// Explicit exhaustive name mapping in place of CH `magic_enum::enum_name`.
        ...
        void resetFlag(State from_state, State to_state = State::Active) noexcept
        {
            ...
            chassert(
                prev == from_state,
                printUnexpectedState(prev, stateName(from_state), fmt::format("{}", stateName(to_state))));
        }
    ...
    private:
            ...
            State prev_state, std::string_view expected_state, std::string type) const
        {
            return fmt::format(
                ...
                stateName(prev_state), expected_state, type, toString());
        }
