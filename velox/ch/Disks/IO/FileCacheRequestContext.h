...
/// Request-scoped identity for a single scan read. Unlike `FileCacheReadOptions`
/// (per-read behavior) and `FileCacheConfig` (per-cache lifetime), this value
/// carries the query/user identity used to derive the ClickHouse
/// `FileCacheOriginInfo` and the query-limit context.
///
...
///
/// `userId` must not be derived from `driverId`/`scanId`; `ConnectorQueryCtx`
/// currently provides no real user identity, so the stable Manager identity is
/// used until a real user-mapping resolver is injected.
