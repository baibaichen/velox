...
// ============================================================================
// ch-task9a: ch HashMethod 首次接坐标模型 + pipeline (验证 spike)。
//
// 这条是 task9 的 "ch 路径"，与现有 ch decoder 路径 (ChHashBuild/ChHashProbe)
// 并存、独立、opt-in——ch 库一个字不改，默认永远走旧路径，只有显式调用本文件
// 的函数才走 ch HashMethod 驱动。9a 只加不删不切不改名。
//
// 咬合点 (task9a 真难点):让 ch 的 emplaceKey/findKey 驱动**在坐标模型上跑**。
//   - mapped 承载:复用现有 ch 坐标 map 类型 `ch::HashMapAll_*` (cell mapped =
//     ch::RowRefList)。ch HashMethod 的 `Data` 模板挂 ch::HashMapAll_key64 /
//     _key_string / _hashed，`Mapped` = ch::RowRefList——ch base 的
//     emplaceImpl 在插入时 `new (&it->getMapped()) RowRefList()` 默认构造空
//     RowRefList (word=0)，正是坐标字的空列表。ch 换 mapped 类型即可，
//     无需改 ch HashMethod 的 key 处理算法 (铁律:算法不动)。
//   - build 插坐标:ch emplaceKey 拿到 cell 后，对 cell 的 RowRefList 做
...
// ch 坐标 build:用 ch HashMethod 的 emplaceKey 驱动，往 ch 坐标 map
...
void chBuildCoordinates(
  ...
  static_assert(
      ...
      "ch coordinate build requires a ch::RowRefList-mapped map");
  ...
  for (size_t row = 0; row < rows; ++row) {
    ...
    // ch base 插入时已 `new (&getMapped()) RowRefList()` 默认构造空列表。
...
// ch 坐标 probe:用 ch HashMethod 的 findKey 驱动，命中后拿 RowRefList& 收成
// ch::ProbeHit (对齐 ch joinProbe 的 hit 收集)。返回的 ProbeHit 直接喂
// ch::listJoinResults + ch::EmitGather。
template <typename HashMethod, typename CoordinateMap>
std::vector<ch::ProbeHit> chProbeCoordinates(
    HashMethod& hashMethod,
    CoordinateMap& coordinateMap,
    ch::Arena& arena,
