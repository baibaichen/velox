...
SplitFileCachePriority::SplitFileCachePriority(
    ...
    , systemSegmentSizeRatio(system_segment_size_ratio_)
    , maxDataSegmentSize(getRatio(max_size_, (1 - systemSegmentSizeRatio)))
    , maxDataSegmentElements(getRatio(max_elements_, (1 - systemSegmentSizeRatio)))
    , maxSystemSegmentSize(getRatio(max_size_, systemSegmentSizeRatio))
    , maxSystemSegmentElements(getRatio(max_elements_, systemSegmentSizeRatio))
{
    prioritiesHolder[static_cast<uint8_t>(SegmentType::Data)] = creator_function(
        maxDataSegmentSize,
        maxDataSegmentElements,
        size_ratio,
        0, // Overcommit available only for CH Cloud
        description_ + "_" + getKeyTypePrefix(SegmentType::Data));
    prioritiesHolder[static_cast<uint8_t>(SegmentType::System)] = creator_function(
        maxSystemSegmentSize,
        maxSystemSegmentElements,
        size_ratio,
        0, // Overcommit available only for CH Cloud
    ...
    const CacheStateGuard::Lock & lock)
{
    ...
    getPriority(SegmentType::Data).modifySizeLimits(
        maxDataSegmentSize, maxDataSegmentElements, size_ratio_, lock);

    getPriority(SegmentType::System).modifySizeLimits(
        maxSystemSegmentSize, maxSystemSegmentElements, size_ratio_, lock);
    ...
    const CacheStateGuard::Lock & lock)
{
    ...
    size_t desired_data_size = getRatio(desired_max_size, 1 - systemSegmentSizeRatio);
    size_t desired_system_size = getRatio(desired_max_size, systemSegmentSizeRatio);
    size_t desired_data_elements = getRatio(desired_max_elements, 1 - systemSegmentSizeRatio);
    size_t desired_system_elements = getRatio(desired_max_elements, systemSegmentSizeRatio);
...
{
    const auto type = getPriorityType(key_metadata->origin.segmentType);
...
{
    const auto type = getPriorityType(origin_info.segmentType);
...
bool SplitFileCachePriority::tryIncreasePriority(
    ...
    CacheStateGuard & state_guard)
{
    const auto type = getPriorityType(iterator.getEntry()->keyMetadata->origin.segmentType);
...
SplitFileCachePriority::SplitIterator::SplitIterator(
    ...
    : type(type_)
    , cachePriority(inner_cache_priority)
    , iterator(iterator_)
