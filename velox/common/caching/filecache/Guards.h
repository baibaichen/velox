...
struct CachePriorityGuard : private boost::noncopyable {
  ...
  ReadLock readLock() {
    // TODO(profile-events): CH recorded FilesystemCachePriorityReadLockMicroseconds here
    return ReadLock(mutex);
  }
  WriteLock writeLock() {
    // TODO(profile-events): CH recorded FilesystemCachePriorityWriteLockMicroseconds here
...
struct CacheStateGuard : private boost::noncopyable {
  ...
  Lock lock() {
    // TODO(profile-events): CH recorded FilesystemCacheStateLockMicroseconds here
    return Lock(mutex);
  }
