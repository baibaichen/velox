...
class FileCacheDriver {
 public:
  FileCacheDriver(uint64_t workingSetKeys, uint64_t latencyUs, int cellIdx)
    ...
    cache_ = std::make_unique<FileCache>("filecache_bench", makeSettings(cacheRoot_));
    cache_->initialize();
  }
  ...
  SleepyReadFile& readFile() { return sleepyReadFile_; }
...
bool downloadSegment(FileSegment& segment, SleepyReadFile& readFile) {
  const auto state = segment.state();
  if (state == FileSegment::State::DOWNLOADED) {
    return true;
  }
  if (state == FileSegment::State::DETACHED) {
    std::vector<char> bypass(segment.range().size());
    readFile.pread(segment.range().left, segment.range().size(), bypass.data());
    return false;
  }
  ...
  if (downloaderId != FileSegment::getCallerId()) {
    while (segment.getDownloadedSize() < segment.range().size() &&
           segment.state() == FileSegment::State::DOWNLOADING) {
      segment.wait(segment.range().right);
    }
    return segment.state() == FileSegment::State::DOWNLOADED;
  }

  try {
    constexpr uint64_t kChunk = 1ULL << 20;
    ...
    while (segment.getDownloadedSize() < segment.range().size()) {
      ...
      if (!segment.reserve(toRead, kReserveTimeoutMs, failureReason, nullptr)) {
  ...
  for (uint64_t t = 0; t < threads; ++t) {
    workers.emplace_back([&, t] {
      ...
      for (uint64_t i = 0; i < opsPerThread; ++i) {
        ...
        auto holder = driver.cache().getOrSet(
            FileCacheKey::fromPath(kRemotePath),
            offset,
            kSegmentBytes,
            driver.readFile().size(),
            CreateFileSegmentSettings{},
            0,
            FileCache::getCommonOrigin());
        // hit == every segment was already DOWNLOADED when getOrSet returned.
        // Sampled before the download below. Under concurrency this skews
        // slightly low on contended cold cells: when two threads grab the same
        // cold key simultaneously both observe a non-DOWNLOADED state and count
        // as misses even though only one will perform the remote read.
        bool opHit = true;
        for (const auto& segment : *holder) {
          opHit = opHit && segment->state() == FileSegment::State::DOWNLOADED;
        }
        for (const auto& segment : *holder) {
          (void)downloadSegment(*segment, driver.readFile());
        }
        if (opHit) {
          ++stats.hits;
        } else {
...
CellResult runCell(
  ...
  FileCacheDriver driver(wsKeys, key.latencyUs, cellIdx);
  ...
  driver.readFile().resetBytesRead();
  ...
  r.bytesDlMB = driver.readFile().bytesRead() / (1ULL << 20);
